...
const AvailableProducts = () => {
  ...
  useEffect(() => {
    fetch("/api/retailer/available-products")
      .then((res) => res.json())
      .then((result) => {
  ...
  return (
    <div className="flex flex-col items-center text-gray-800 dark:text-white">
      ...
      <div className="w-full max-w-3xl space-y-4">
        {products.map((prod) => (
          <motion.div key={prod._id} className="p-4 border-l-4 border-green-500 bg-green-50 dark:bg-green-950 rounded-md shadow-md"
          ...
          >
            ...
            <ul className="list-disc list-inside ml-2 text-sm">
              {prod.batches?.map((b, i) => (
                <li key={i}>
                  Qty: {b.quantity} | Expiry: {new Date(b.expiry).toLocaleDateString()}
                </li>
              ))}
            </ul>
          </motion.div>
